...
from .cfg import AnymalCNavRoughEnvCfg
from .common import generate_repeating_array


@registry.env("anymal_c_navigation_rough", "np")
class AnymalCRoughEnv(NpEnv):
    _cfg: AnymalCNavRoughEnvCfg

    def __init__(self, cfg: AnymalCNavRoughEnvCfg, num_envs: int = 1):
    ...
    def update_state(self, state: NpEnvState):
        ...
        # Border check: if out of bounds, stop moving
        is_out = self.check_border(data)
        velocity_commands[is_out] = 0.0
    ...
    def reset(self, data: mtx.SceneData, done: np.ndarray = None) -> tuple[np.ndarray, dict]:
        ...
        # Curriculum learning for initial position
        if self.training_level == 1:
            num_period = 25
            idx = generate_repeating_array(num_period, num_envs, self.height_counter)
            self.height_counter = (self.height_counter + num_envs) % num_period
            
            # Use offset list for position (x, y, z)
            selected_offsets = self.offset_list[idx]
            robot_init_x = selected_offsets[:, 0]
            robot_init_y = selected_offsets[:, 1]
            # Z will be handled later
        ...
        # Override Z if curriculum active
        if self.training_level == 1:
             dof_pos[:, 2] = self.offset_list[idx][:, 2]
